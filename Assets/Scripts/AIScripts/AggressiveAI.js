...
function createRaycast(relativeVelocity : Vector3) {
	...
	raycastY = carBody.position.y; // Put the starting point of the raycast a little bit higher
	raycastY += 1;
    ...
    if(Physics.Raycast(raycastStartPos, (raycastFront.transform.position - raycastStartPos), hitFront, 15)) { // Constructs the actual ray
...
function AttackCar(hit : RaycastHit) {
...
function followWaypoints(waypoint : Array, relativeVelocity : Vector3, laps : int) {
	...
	if(currentWaypoint < waypoint.length) { // Iterate trough the waypoints
		...
		if(moveDirection.magnitude < 6) { // If the bot is close to the waypoint, change to next one
			...
			if(foundACar) { //If raycast detects a car, attack it
				...
				//	transform.rigidbody.velocity = targetsVelocity; // Make the AI go with the speed of the target
				
					velocityDifference = transform.rigidbody.velocity.magnitude - targetsVelocityMagnitude; // The difference in the speed of both the player and the bot
